...
contract NFTArena is ERC1155 {
    ...
    function _mintPlayer() external {
        ...
        quests[playerCount] = Quest (
            0
        );
        trainings[playerCount] = Train (
            0
        );
    }
    ...
    function uri(uint256 _id) public view override returns (string memory) {
        ...
        string memory s = string(abi.encodePacked(baseURI, Strings.toString(_id)));
        return s;
    }
    ...
    function endQuest(uint256 _tokenId) external {
        require(block.timestamp >= quests[_tokenId].endTime, "It's not time to finish quest");
        setIdle(_tokenId);
    ...
    function endTraining(uint256 _tokenId) external {
        require(block.timestamp >= trainings[_tokenId].startTime + 120, "it's too early to pull out");
        setIdle(_tokenId);
